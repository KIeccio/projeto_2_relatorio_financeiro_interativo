...
def create_gold_product_table(data_processamento:str) -> None:
    ...
    df_vendas_sumarizado = (
        df_vendas.groupBy(
            ...
            F.round(F.sum(F.col('valor_unitario')), 2).alias('TotalValorVendido'),
            F.round(
                F.sum(F.col('valor_unitario')) / F .sum(F.col('quantidade')), 2).alias('TicketMedio')
        )
        .orderBy('CodigoData', 'ProdutoId', 'FilialId')
    )
...
# create_gold_product_table('2025-09-19')

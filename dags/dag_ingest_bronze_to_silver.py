...
with DAG(
    ...
    ingest_vendas = PythonOperator(
        task_id='ingest_vendas_bronze_to_silver',
        python_callable=run_vendas
    )

    ingest_estoque = PythonOperator(
        task_id='ingest_estoque_bronze_to_silver',
        python_callable=run_estoque
    )

    ingest_devolucao = PythonOperator(
        task_id='ingest_devolucao_bronze_to_silver',
        python_callable=run_devolucao
    )

    ingest_frete = PythonOperator(
        task_id='ingest_frete_bronze_to_silver',
        python_callable=run_frete
    )

    ingest_tributos = PythonOperator(
        task_id='ingest_tributos_bronze_to_silver',
        python_callable=run_tributos
    )

    [ingest_vendas, ingest_estoque, ingest_devolucao, ingest_frete, ingest_tributos]

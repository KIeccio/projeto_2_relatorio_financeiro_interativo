...
BASE_DIR = '/home/kleccio/Área de trabalho/portfolio' # Caminho base Datalake
...
with DAG(
    ...
    def ingest_devolucao_bronze_to_silver(data_processamento:str) -> None:
    ...
    def ingest_estoque_bronze_to_silver(data_processamento:str) -> None:
        ingest_bronze_to_silver(
    ...
    def ingest_vendas_bronze_to_silver(data_processamento:str) -> None:
        ingest_bronze_to_silver(
    ...
    def ingest_frete_bronze_to_silver(data_processamento:str) -> None:
        ingest_bronze_to_silver(
    ...
    def ingest_tributos_bronze_to_silver(data_processamento:str) -> None:
        ingest_bronze_to_silver(
    ...
    def create_gold_product_table(data_processamento:str) -> None:
        ...
        try:
            ...
            if 'DELTA_MISSING_DELTA_TABLE' in str(e):
                print('Tabela Delta nao encontrada na camada prata. Realizando a criacao de uma tabela nova.')
                RECRIAR_TABELA = True
    ...
    def create_gold_freight_table(data_processamento:str) -> None:
        ...
        if RECRIAR_TABELA:
            ...
            df_fretes = spark.read.format('delta').load(f"{SILVER_PATH}/frete/frete").filter(f"data_pagamento= '{data_processamento}'")
    ...
    operator_devolucao_bronze_to_silver = PythonOperator(
        task_id = "INGEST_DEVOLUCAO_BRONZE_TO_SILVER",
        python_callable=ingest_devolucao_bronze_to_silver,
    ...
    operator_estoque_bronze_to_silver = PythonOperator(
        task_id = "INGEST_ESTOQUE_BRONZE_TO_SILVER",
        python_callable=ingest_estoque_bronze_to_silver,
        provide_context=True,
        op_kwargs = {
            'data_processamento': "{{ ds }}"
        }
    )
    operator_vendas_bronze_to_silver = PythonOperator(
        task_id = "INGEST_VENDAS_BRONZE_TO_SILVER",
        python_callable=ingest_vendas_bronze_to_silver,
        provide_context=True,
        op_kwargs = {
            'data_processamento': "{{ ds }}"
        }
    ...
    operator_frete_bronze_to_silver = PythonOperator(
        task_id = "INGEST_FRETE_BRONZE_TO_SILVER",
        python_callable=ingest_frete_bronze_to_silver,
        provide_context=True,
        op_kwargs = {
            'data_processamento': "{{ ds }}"
        }
    ...
    operator_tributos_bronze_to_silver = PythonOperator(
        task_id = "INGEST_TRIBUTOS_BRONZE_TO_SILVER",
        python_callable=ingest_tributos_bronze_to_silver,
        provide_context=True,
        op_kwargs = {
            'data_processamento': "{{ ds }}"
        }
...
[
    operator_devolucao_bronze_to_silver, 
    operator_vendas_bronze_to_silver, 
    operator_estoque_bronze_to_silver
] >> operator_create_gold_product_table
operator_frete_bronze_to_silver >> operator_create_gold_freight_table
operator_tributos_bronze_to_silver >> operator_create_gold_taxes_table
